...
// Language: rust
//
// In Rust there are two main types of strings: string slices (`&str`) and
// the `String` type. String slices are references to some UTF-8 encoded
// string data stored elsewhere. String literals are stored in the program's
// binary and are therefore string slices. The `String` type, on the other hand,
// is allocated on the heap and is therefore able to store an amount of text
// that is unknown to us at compile time.

fn main() {

    // String literal, fixed size and immutable string slice, and are stored in
    // the program's binary as immutable data.
    let literal: &str = "hello, world!";
    println!("{}", literal);

    // String slice, an immutable reference to a UTF-8 encoded string in memory
    let slice: &str = &literal[..5];
    println!("{}", slice);
